...
public class Celvin extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_celvin);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("CELVIN KLEIN");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Celvin.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Celvin.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Celvin.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Celvin.ItemViewHolder viewHolder, Jeans model, int position) {
