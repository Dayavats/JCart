...
public class Spykar extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_spykar);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("SPYKAR");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Spykar.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Spykar.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Spykar.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Spykar.ItemViewHolder viewHolder, Jeans model, int position) {
