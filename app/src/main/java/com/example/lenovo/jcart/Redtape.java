...
public class Redtape extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_redtape);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("REDTAPE");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Redtape.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Redtape.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Redtape.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Redtape.ItemViewHolder viewHolder, Jeans model, int position) {
