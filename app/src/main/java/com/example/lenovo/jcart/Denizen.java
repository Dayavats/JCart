...
public class Denizen extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_denizen);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("DENIZEN");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Denizen.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Denizen.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Denizen.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Denizen.ItemViewHolder viewHolder, Jeans model, int position) {
