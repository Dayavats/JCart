...
public class Johnp extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_johnp);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("JOHN PLAYERS");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Johnp.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Johnp.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Johnp.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Johnp.ItemViewHolder viewHolder, Jeans model, int position) {
