...
public class Cantabil extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_cantabil);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("CANTABIL");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Cantabil.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Cantabil.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Cantabil.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Cantabil.ItemViewHolder viewHolder, Jeans model, int position) {
