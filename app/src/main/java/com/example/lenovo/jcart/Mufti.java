...
public class Mufti extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_mufti);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("MUFTI");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Mufti.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Mufti.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Mufti.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Mufti.ItemViewHolder viewHolder, Jeans model, int position) {
