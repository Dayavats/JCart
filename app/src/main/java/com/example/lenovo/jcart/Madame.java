...
public class Madame extends AppCompatActivity {
    private RecyclerView list;
    private Query mref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_madame);
        list=(RecyclerView)findViewById(R.id.rv);
        list.setHasFixedSize(true);
        list.setLayoutManager(new LinearLayoutManager(this));
        mref= FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("brand").equalTo("MADAME");
    ...
    protected void onStart() {
        super.onStart();
        FirebaseRecyclerAdapter<Jeans,Madame.ItemViewHolder> FBRA=new FirebaseRecyclerAdapter<Jeans,Madame.ItemViewHolder>(
                Jeans.class,
                R.layout.view_layout,
                Madame.ItemViewHolder.class,
                mref
        ) {
            @Override
            protected void populateViewHolder(Madame.ItemViewHolder viewHolder, Jeans model, int position) {
